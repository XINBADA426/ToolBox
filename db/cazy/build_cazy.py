...
@click.option('-f', '--fasta',
              required=True,
              type=click.Path(),
              help="The input fasta file download from dbCAN")
@click.option('-o', '--out',
              default="./",
              show_default=True,
              type=click.Path(),
...
def cli(fasta, out, diamond, threads):
    """
    Script to build CAZy database
    """
    logging.info(f"Start to parse {fasta}...")
    out_faa = os.path.join(out, "CAZy.faa")
    out_tab = os.path.join(out, "seq_levelB.info.txt")
    with open(out_faa, 'w') as OUT1, open(out_tab, 'w') as OUT2:
        for record in SeqIO.parse(fasta, 'fasta'):
            arr = record.name.strip().split('|')
            print(f">{arr[0]}\n{record.seq}", file=OUT1)
            print(*arr, sep='\t', file=OUT2)
    logging.info(f"Build the diamond database...")
    out_pre = os.path.join(out, "CAZy")
